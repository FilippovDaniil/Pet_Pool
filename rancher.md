...
## Billiard Club (Pet_Pool) — параметры стека

Namespace: `billiard-club` · Порт приложения: `7070` · NodePort: `30707`

| Сервис | Тип | NodePort | URL |
|--------|-----|----------|-----|
| billiard-app | NodePort | 30707 | http://localhost:30707 |
| postgres | ClusterIP | — | postgres:5432 |
| opensearch | ClusterIP | — | opensearch:9200 |

**Workflow обновления:**
```powershell
# Пересобрать и задеплоить (из корня проекта)
.\rancher\build-and-load.ps1 -Restart

# Первый деплой (после kubectl apply -f rancher/k8s/)
...
1. **`/actuator/health` → 401**: K8s probes не авторизованы. Нужно `permitAll()` для этого пути в SecurityConfig.  
   Симптом: `Readiness probe failed: HTTP probe failed with statuscode: 401` → бесконечный restart.

2. **Dockerfile multi-stage внутри Docker → timeout**: Gradle wrapper скачивает дистрибутив из интернета внутри build-контейнера. При переключении Docker context-а (Rancher Desktop → Docker Desktop) кеш слоёв сбрасывается → повторная загрузка → timeout.  
   Решение: Строить JAR на хосте (`./gradlew bootJar`), Docker только копирует готовый JAR. `.dockerignore` — `build/` + `!build/libs/*.jar`.

3. **`@WebMvcTest` + `SecurityConfig`**: `@WebMvcTest` не загружает `@Configuration` бины. Без `@Import(SecurityConfig.class)` security-правила (403 для не-ADMIN, 401 для неавторизованных) не применяются → тесты дают неверные статусы.

4. **`formLogin` redirect**: Spring Security form-login отвечает 302 (redirect to /login) вместо 401 для REST эндпоинтов. Нужен отдельный `exceptionHandling.defaultAuthenticationEntryPointFor` для `/api/**`.
