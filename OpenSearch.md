...
## Статус в проекте Pet_Pool (Billiard Club)
...
| `ClientDocument` | `search/ClientDocument.java` | Документ клиента для индексирования |
| `ClientSearchService` | `search/ClientSearchService.java` | Интерфейс |
| `ClientSearchServiceImpl` | `search/ClientSearchServiceImpl.java` | Реализация (паттерн B + null-guard) |
| `SearchInitializer` | `config/SearchInitializer.java` | Реиндексация при старте (`@Order(2)`, `@Transactional(readOnly=true)`) |
| `SearchController` | `controller/SearchController.java` | `GET /api/search/clients` |

**Эндпоинт:** `GET /api/search/clients?q=&rank=&page=&size=`  
**Разрешение:** `permitAll()`, CSRF отключён для `/api/search/**`  
**Тесты:** `opensearch.enabled=false` в `application-test.properties` — все тесты без OpenSearch  
**Индекс:** `clients` — поля: `fullName^2`, `rank`, `phone`  
**Graceful degradation:** при `client == null` все методы возвращают no-op / пустой список  
**K8s namespace:** `billiard-club`, сервис: `opensearch:9200`

---

## Статус в проекте Pet_Gym
...
|---|---|---|
| `OpenSearchConfig` | `config/OpenSearchConfig.java` | Бин клиента (Вариант B) |
| `TrainerDocument` | `search/TrainerDocument.java` | Документ тренера для индексирования |
| `TrainerSearchService` | `search/TrainerSearchService.java` | Интерфейс |
| `TrainerSearchServiceImpl` | `search/TrainerSearchServiceImpl.java` | Реализация (паттерн B + null-guard) |
| `SearchInitializer` | `search/SearchInitializer.java` | Реиндексация при старте (`@Order(2)`, `@Transactional(readOnly=true)`) |
| `SearchController` | `controller/SearchController.java` | `GET /api/search/trainers` |

**Эндпоинт:** `GET /api/search/trainers?q=&specialization=&page=&size=`  
**Разрешение:** `permitAll()` (без авторизации, как и GET /api/client/trainers)  
**Тесты:** `opensearch.enabled=false` в `application-test.properties` — все тесты без OpenSearch  
**Индекс:** `trainers` — поля: `firstName^2`, `lastName^2`, `specialization`, `bio`, `email`  
**Graceful degradation:** при `client == null` все методы возвращают no-op / пустой список
...
| 16 | `MatchQuery.query(String)` вместо `FieldValue` | Ошибка компиляции в opensearch-java 2.x | `.query(FieldValue.of(value))` |
| 17 | `@Transactional(readOnly=true)` отсутствует в `SearchInitializer.run()` | `LazyInitializationException` при доступе к `trainer.getUser()` | Добавить `@Transactional(readOnly=true)` на `run()` — держит JPA-сессию |
...
## Известные проблемы в Pet_Pool (Billiard Club)

| # | Проблема | Симптом | Решение |
|---|----------|---------|---------|
| 1 | `/actuator/health` требует авторизации | K8s readiness/liveness probe получает 401 → `Unhealthy` → перезапуск пода | Добавить `.requestMatchers("/actuator/health").permitAll()` в `SecurityConfig` ПЕРЕД другими правилами |
| 2 | Dockerfile multi-stage: `./gradlew` внутри Docker не имеет интернета | `IOException: Downloading gradle-X.X-bin.zip failed: timeout` при `docker build` | Строить JAR локально (`./gradlew bootJar -x test`), Dockerfile только копирует `build/libs/*.jar`; `.dockerignore` исключает `build/` но пропускает `!build/libs/*.jar` |
| 3 | `@WebMvcTest` не загружает `SecurityConfig` | Тесты на security-правила (403, 401) дают неверные статусы | Добавить `@Import(SecurityConfig.class)` ко всем `@WebMvcTest` тестам |
| 4 | `formLogin` redirect vs 401 для REST | Неавторизованный запрос к `/api/**` → 302 redirect вместо 401 | `exceptionHandling.defaultAuthenticationEntryPointFor(sendError(401), "/api/**")` в `SecurityConfig` |

---

## Известные проблемы в Pet_Gym

| # | Проблема | Симптом | Решение |
|---|----------|---------|---------|
| 16 | `httpclient5` без версии, Boot 3.2.x | `ClassNotFoundException` | В Boot 3.4.4 BOM управляет — версию не указывать |
| 17 | `build-and-load.ps1` encoding в PowerShell 5.1 | ParseException (кириллица в комментариях) | Запускать шаги вручную: `docker build --provenance=false` → `docker save` → `rdctl shell -- sh -c "docker load < ..."` → `kubectl apply -f rancher/k8s/` |
| 18 | `MatchQuery.query(String)` | Ошибка компиляции в opensearch-java 2.x | `.query(FieldValue.of(specialization))` — не String, а FieldValue |
| 19 | `SearchInitializer` видит 0 тренеров | `DataInitializer` ещё не отработал | `@Order(1)` на DataInitializer, `@Order(2)` на SearchInitializer — Spring гарантирует порядок |
| 20 | `LazyInitializationException` в SearchInitializer | Сессия JPA закрыта при доступе к `trainer.getUser()` | `@Transactional(readOnly = true)` на методе `run()` — держит сессию открытой |
